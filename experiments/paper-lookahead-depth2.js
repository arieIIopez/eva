/* EVA EDTR · look-ahead de dos pasos sobre cartera completa C/I.
   ...
   En cada estado, para cada candidato p se calcula:
     L2(p|Gt)=S(p|Gt)+delta*max_q S(q|T_p(Gt))
   y se selecciona el p con mayor L2. No es optimizacion global.
*/
(function(){
  'use strict';
  ...
  const projectProps=id=>(projectGeom(id)&&projectGeom(id).properties)||{};
  ...
  async function chooseDepth2(state,locked,def,params,scales,root,ids,discount){
    ...
    for(let i=0;i<state.ranked.length;i++){
      const p=state.ranked[i], g=projectGeom(p.id); if(!g) continue;
      const next=evaluate([...locked,g],def.weights,params,scales,root,ids).ranked[0];
      const v=num(p.score)+discount*num(next&&next.score);
      if(!best||v>best.two_step_value+1e-12||(Math.abs(v-best.two_step_value)<=1e-12&&String(p.id).localeCompare(String(best.chosen.id))<0)) best={chosen:p,two_step_value:v,next_best_score:num(next&&next.score),next_best_id:next&&next.id,evaluated_first_choices:state.ranked.length};
      if(window.evaYield&&i%8===0) await window.evaYield();
    }
    return best;
  }
  async function simulate(def,defs,params,scales,root,ids,discount,mode){
    const locked=[],rows=[]; const n=ids.size;
    for(let step=1;step<=n;step++){
      const state=evaluate(locked,def.weights,params,scales,root,ids);
      let choice;
      if(mode==='depth2') choice=await chooseDepth2(state,locked,def,params,scales,root,ids,discount);
      else choice={chosen:state.ranked[0],two_step_value:null,next_best_score:null,next_best_id:null,evaluated_first_choices:1};
      const p=choice.chosen; if(!p) break; const g=projectGeom(p.id); const x=crossScores(p,defs,scales); locked.push(g); const prev=rows.at(-1)||{};
      ...
      if(step===1||step%10===0||step===n) console.log(`[paper-L2] ${def.label} ${mode}: ${step}/${n} · ${p.id}`);
  ...
  async function run(opts){ opts=opts||{}; const discount=opts.discount==null?0.95:+opts.discount; const params={...(window.PARAM_DEFAULTS||{}),perfil:'general',segKSI:false}; const root=base.rootCfg('Alameda',100,0.5); const ids=eligibleIds(); const defs=scenarios(); setRoot(root); const baseState=base.evaluateState([],allWeights(),params); const scales=scalesFrom(baseState.enriched,ids); const initialComponents=components([],params); const greedy=[],depth2=[];
    console.log(`[paper-L2] Inicio · ${ids.size} proyectos × 3 W · greedy vs depth2`);
    for(const d of defs){ greedy.push(await simulate(d,defs,params,scales,root,ids,discount,'greedy')); depth2.push(await simulate(d,defs,params,scales,root,ids,discount,'depth2')); }
    const gs=greedy.map(r=>summary(r,initialComponents)), ls=depth2.map(r=>summary(r,initialComponents));
    return {generated_at:new Date().toISOString(),design:{eligible_projects:ids.size,scenarios:defs.map(d=>({key:d.key,label:d.label,weights:d.weights})),normalization:'fixed-G0-eligible-CI',root,discount_per_step:discount,lookahead_depth:2,selection_rule:'maximize S_t(p)+delta*max_q S_{t+1}(q|T_p(G_t)) over all currently feasible first choices',optimality_note:'depth-2 rollout; no garantiza óptimo global'},normalization_reference:scales,initial_components:initialComponents,greedy_summaries:gs,depth2_summaries:ls,improvements:gs.map((g,i)=>improvement(g,ls[i])),order_comparison:greedy.map((g,i)=>compare(g,depth2[i])),greedy_runs:greedy,depth2_runs:depth2};
  }
